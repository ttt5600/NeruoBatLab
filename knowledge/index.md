...
Generated 2026-09-25 by `build.py`. Do not edit by hand; edit `findings/*.yaml`.
...
**Method.** Identical architecture (torchaudio wav2vec2_model from the released AVES config; 320-sample hop, 12 blocks, 768-d), identical frame grid, identical StandardScaler + logistic probe, identical CV. AVES was run BOTH with run11's chunk normalisation and with its own raw-waveform preprocessing so the comparison does not smuggle in a preprocessing change; the two differ by <0.002 AUC. Layer selection pre-committed on ZF (run11 L0, AVES L6); every layer reported.
...
**Provenance.** zfeval/experiments/aves_baseline.py, aves_holdout.py; analysis/aves_baseline.json, analysis/aves_holdout.json
...
**Method.** Both encoders read out by the same probe on the same folds; the stacker is fitted out-of-fold so it cannot see its inputs' labels. Layers pre-committed on ZF.
...
**Provenance.** zfeval/experiments/ensemble_detect.py; analysis/ensemble_detect.json
...
**Method.** 350 donor calls (local SNR >= 10 dB, 40-250 ms) windowed to zero outside their own energy support with a 1 ms raised-cosine taper, mixed at known sample offsets into call-free background taken >= 200 ms from any annotation in the one exhaustively annotated recording. Probe trained ONLY on real frames from the first 60% of the recording; donors and background drawn from the last 40%, so nothing in the synthetic audio was seen in probe training. Decoder held fixed across all SNRs.
...
**Provenance.** zfeval/experiments/synth_timing.py (--truth-mode core|tails); analysis/synth_timing.json, analysis/synth_timing_tails.json
...
**Method.** AVES put through run11''s own zf_hubert.embed_file path -- raw audio with NO waveform normalisation, mono channel mean, 16 kHz, mean-pooled over time, all 12 layers. run11 embeddings read from the released layersweep file rather than recomputed. The released cohort was reproduced exactly by dropping the Unknown* placeholder birds (2867 -> 2814 clips, 31 -> 26 birds).
...
**Provenance.** zfeval/experiments/aves_calltype.py; analysis/aves_calltype.json
...
**Method.** Candidate sets were the 12 single layers plus 7 multi-layer combinations. Each outer fold ran its own 3-fold inner CV over all 19 candidates and only then touched its test rows, so no selection can leak. Bootstraps cluster over birds (call type) or recording sessions (identity).
...
**Provenance.** zfeval/experiments/run11_win_search.py; analysis/run11_win_search.json
...
**Method.** AdamW, encoder LR 1e-5, head LR 1e-3, max 8 epochs with patience 3, effective batch 8 via gradient accumulation. No run diverged; train loss fell from ~1.1-1.3 to 0.03-0.16 in all 30 folds. Frozen sanity arm reproduced the published probe (run11 L3 0.8157 vs 0.8118; AVES L3 0.8388 vs 0.8453, the small offsets being AdamW-head vs LogisticRegression+StandardScaler).
...
**Provenance.** zfeval/experiments/finetune_calltype.py; analysis/finetune_calltype.json
...
**Method.** Cause is architectural, not a library bug: extractor_mode="group_norm" places a GroupNorm(512, 512) inside conv block 0, which normalises over TIME. Zero padding changes each clip's time statistics, so the normalisation output shifts at every frame including valid ones. `lengths` only masks transformer attention, which is downstream of the CNN.
...
**Provenance.** independently reproduced in-session; zfeval/experiments/finetune_calltype.py (masked_feature_extractor)
...
**Method.** Config extracted by stubbing fairseq/omegaconf modules and torch.load(weights_only=False), then reconciled field by field against the ICASSP 2023 paper. No disagreement was found on any quantity both report; the config supplies several the paper omits.
...
**Provenance.** docs/aves_method_and_transfer_literature.md; datasets/11905533/aves-base-bio.pt cfg block
...
**Method.** MiniBatchKMeans on standardised features, same frames, same seed, scored with AMI because NMI and purity rise monotonically with k and would hand k=200 a free win.
...
**Provenance.** zfeval/experiments/target_quality.py; analysis/target_quality.json
...
**Method.** Architecture claim verified by reading the code, not inferred: the model is torchaudio.models.hubert_pretrain_base (lightning_modules.py:467), raw waveform in; --feature-type accepts only {spectrogram, hubert} (train.py:119) and flows into CollateFnHubert and utils/feature_utils.py, never into the model; the sole surviving MFCC branch in the codebase is `label = label[::2]` (dataset/hubert_dataset.py:527), a frame-rate halving of the LABEL sequence because Kaldi MFCC runs at 10 ms against HuBERT frames at 20 ms. The run11 feature was produced by calling the production extractor utils.feature_utils.extract_feature_spectrogram itself rather than a reimplementation. Frame alignment is exact and asserted at runtime (HuBERT frame i, soundsig window i and Kaldi MFCC frame 2i all cover samples [320i, 320i+400)); the chunked MFCC path is cross-checked against a contiguous call across a chunk boundary (max|diff| < 1e-3). Constant columns are dropped before z-scoring -- the extractor takes log10(power + 1e-8) so silent bins pin at exactly -8.0, and StandardScaler turns those into inf and nan centroids silently.
...
**Provenance.** zfeval/experiments/target_features.py; analysis/target_features.json
...
**Method.** Frozen features, readout swapped, split and bootstrap unit held identical to [033]. Two seeds on every learned head. Guarded by three checks recorded in the result file: batched torchaudio extraction is off by 25.80 at valid frames while the masked extractor is off by 2.1e-05 (see [037]), batch-vs-single head agreement is <1e-06 for all four heads, and masked-vs-unmasked attentive pooling differs by 1.22, which is why masking is not optional here.
...
**Provenance.** zfeval/experiments/attentive_probe.py; analysis/attentive_probe.json
...
**Method.** Two-stage selection to keep the epoch count out of the test fold: stage A fits on an inner 80% and picks the epoch by validation AUC against HARD labels, stage B retrains on the full training fold for exactly that many epochs. Teacher probabilities are per-fold LogisticRegression outputs averaged across the two encoders; soft targets are in-sample train probabilities. AdamW + cosine, 400 epochs, batch 1024, lr 1e-3, temperature 1.0, w_kl_ce 0.5, seed 0.
...
**Provenance.** zfeval/experiments/distillation.py; analysis/distillation.json, analysis/distillation_exp2.json
...
**Method.** run11's per-step audio is no longer taken from the --gpus flag, which SLURM never honoured, but derived from what the run recorded: consecutive checkpoint (epoch, step) pairs on scratch give 5201/5202/5201 steps per epoch, and the training TSV gives 417,702.83 s of audio, so 417,702.83 / 5201.33 = 80.3 s/step. world_size is read from #SBATCH --ntasks (what srun launched), not --gpus (what the job asked Lightning for); the script asserts the realised batch does not exceed the nominal 87.5 cap. AVES unchanged, from the fairseq cfg inside the 2022 checkpoint: max_tokens 1.4e6 / 16000 x update_freq 8 x world_size 1 = 700 s. The self-check now runs for BOTH models and asserts on each: derived epochs 18.02 vs Lightning's recorded 18 (0.1% apart) for run11, 54.01 vs fairseq's 55 (1.8%) for AVES.
...
**Provenance.** zfeval/experiments/compute_budget.py; analysis/compute_budget.json
...
**Method.** zfeval/experiments/detection_variants.py, extended to load DAPT exports so they inherit its doctrine unchanged: both normalisations scored for every checkpoint, layer pre-committed by out-of-fold ZF AUC and never chosen on BirdPark, all 12 arms written to JSON for audit. Cluster bootstrap over 1500-frame (30 s) and 500-frame blocks. Encoders exported with export_weights.py, which refuses to write unless the rebuilt encoder is bit-identical to the training checkpoint (verified 0.000e+00 on three durations for all 12).
...
**Provenance.** analysis/detection_variants.json; slurm/train_dapt_aves_init.sh; jobs 38991843, 38996893
...
**Method.** Read directly from the full 12-arm grid (2 normalisations x 6 layers) that detection_variants.py already writes for audit; no new computation.
...
**Provenance.** analysis/detection_variants.json (in_distribution and zf_to_bp blocks, all arms)
...
**Method.** zfeval/experiments/detection_variants.py over 12 exported DAPT checkpoints (6 steps x 2 learning rates), arms pre-committed on out-of-fold ZF AUC and frozen before BirdPark is touched, then the existing paired block bootstrap against run11.
...
**Provenance.** zfeval/experiments/detection_variants.py; analysis/detection_variants.json; notebooks/03_forgetting_curve.ipynb
...
**Method.** zfeval/experiments/chick_holdout_variants.py. Arms are read from detection_variants.json's ZF pre-commitment and never chosen on chick data. Clips encoded ONE AT A TIME (group norm over time means padding corrupts short LT clips). 44.1 kHz chick audio polyphase-resampled to 16 kHz. Supervised cohort restricted to the 10 chicks producing BOTH call types, since a single-class chick is a degenerate leave-one-out fold that shifts pooled AUC; AMI uses all 415 clips / 15 chicks. Resampling unit is the CHICK, never the clip -- clips within a chick are not independent and clip-level resampling would produce intervals several times too narrow.
...
**Caveats.** The ranking result rests on FIVE encoders whose true accuracies span only 0.052, so any single rho is one swap from non-significance and individual values of +-0.3 are noise; the claim is carried by the sign consistency across k (10/12 and 9/12), not by any one correlation. All five encoders share an architecture, a dimensionality and a layer, so this tests ranking within a family, not across model classes. Stability's argmax at k=4-25 is partly methodological -- ARI declines with k for generic combinatorial reasons -- so it is suggestive of coarse super-groups consistent with the k=4 HDBSCAN result, NOT evidence that zebra finches have 8 call types. The 1-NN ceiling makes the cluster-vote metric a compression curve, which retrospectively reframes the oracle-k numbers in [project_calltype_clustering] as a statement about how much compression the geometry tolerates rather than about a correct number of clusters. Nothing here says stability would rank encoders correctly on a species with a different call repertoire; it says the procedure survived its one available calibration. A SECOND calibration was attempted on 2026-09-22 and is UNINFORMATIVE -- recorded here so it is not retried. The design inverted target and nuisance on the same 3412 clips: bird identity (48 classes) as the target, call type as the nuisance, predicting that call-type-held-out stability would rank the encoders and that bird-held-out stability would fail as a negative control. Neither prediction could be tested, because the identity target does not resolve these five encoders: bootstrapping over clips at k=450, the top FOUR are pairwise indistinguishable (adjacent 95% CIs all span zero; top-4 spread 0.0070 against a CI width of ~0.031), and only birdaves-biox-base separates (-0.0501 [-0.0657, -0.0337]). The target therefore sorts the encoders into two groups, not five. Quantified directly: the Spearman rho of the TRUE accuracy against itself under resampling has median +0.80 with 95% interval [+0.15, +1.00], so a perfect criterion could score as low as +0.15 here. Every observed rho on this target -- nuisance-held-out +0.25, target-held-out +0.26, silhouette +0.57, Davies-Bouldin +0.58 -- falls inside that interval and none is distinguishable from a perfect criterion or from each other. In particular the apparent reversal, compactness beating stability on identity, is NOT a finding and must not be quoted as one. See [[feedback_no_trends_in_underpowered_data]] and [[feedback_arm_selection_is_underpowered]].
...
### [051] Corpus size and label vocabulary do not explain the AVES advantage  ·  **CONFIRMED**

run15 is run11's exact recipe trained from scratch on 224.42 h (ZF 116.0 h + FSD50K 108.4 h) with a k=200 vocabulary instead of k=100, every other knob held at run11's value. On 11-class call type it scores 0.8136 against run11's 0.8118 -- a difference of +0.0018 with a 95% bird-bootstrap interval of [-0.0169, +0.0243], i.e. NOT DISTINGUISHABLE. Against the AVES family it loses by resolved margins: -0.0317 [-0.0603, -0.0038] vs aves-base-bio, -0.0366 [-0.0575, -0.0126] vs aves-base-core, -0.0384 [-0.0581, -0.0180] vs birdaves-biox-base. Doubling the corpus with exactly the kind of diverse general audio AVES trains on, and doubling the codebook to match AVES, moved nothing. The AVES advantage is therefore NOT attributable to corpus size, corpus diversity, or label vocabulary size at this compute budget.

**Evidence.** Cohort 3412 clips / 11 call types / 48 birds, leave-birds-out StratifiedGroupKFold(5), probe = standardise + multinomial logistic regression on mean-pooled embeddings, each clip encoded at its own length. Best-layer accuracies: run11 0.8118 (L3), run15 0.8136 (L3), aves-base-bio 0.8453 (L3), aves-base-all 0.8479 (L0), aves-base-core 0.8502 (L3), birdaves-biox-base 0.8520 (L1), birdaves-biox-large 0.8552 (L2), birdaves-bioxn-large 0.8453 (L1). run15's per-layer curve is flat and shallow-peaked exactly like run11's -- L0 0.8127, L3 0.8136 (best), L11 0.8033 -- with no sign of the deeper optimum AVES variants show. Intervals are paired bootstraps over the 48 birds, 2000 resamples, the only independent unit here. Corpus verified before training: 763 files, 224.42 h, 40,395,592 label frames at exactly 50.00 fps, all 200 clusters populated, perplexity 75.7 against run11's 31.8.

**Method.** Corpus built by slurm/build_combined_corpus.sh (job 39114877). Training by slurm/train_run15_combined.sh (jobs 39132082 preempted, 39164203 completed, 4 h 09 m, all 93750 updates). The sbatch/srun shape was copied VERBATIM from run11's script, warts included: --gpus 4 under --ntasks=1 does not realise four GPUs, and correcting it would have changed the effective batch and reconfounded the comparison. Verified rather than assumed by steps per epoch -- run11 5208 steps/epoch over 116.03 h = 80.2 s of audio per step, run15 10032 steps/epoch over 224.42 h = 80.5 s. HUBERT_NORMALIZE_INPUT=0 carried over for the same reason (finding 049). Weights exported with export_weights.py, which refuses to write unless the rebuilt encoder is bit-identical; verification reported max|diff| 0.000e+00 at three clip lengths. Scored by zfeval/experiments/run15_calltype.py, reusing aves_variants_calltype's own encode and sweep so the row lands on the same footing as the six AVES rows.

**Caveats.** FIXED COMPUTE, NOT FIXED EPOCHS. run15 made 9.35 passes over its corpus where run11 made 18.0, because both were held to 93750 updates. So the honest statement is "at the same optimiser budget, a 1.93x larger and more diverse corpus buys nothing" -- NOT "corpus size never matters". The fixed-epochs arm (~181k updates) has not been run and would separate those. Reading it the other way is also legitimate and unresolved: run15 reached the same accuracy with HALF the passes per file, which is consistent with the corpus helping and the budget binding. Masked accuracy is NOT comparable across these runs -- run15 predicts among 200 classes and run11 among 100, so run15's lower 0.5404 against run11's ~0.5795 says nothing. All accuracies here are best-layer-selected, which finding [046] shows inflates margins; the ranking is sound but absolute gaps are optimistic for every model alike. What remains unexplained about the AVES gap is now narrower: iteration-2 labels (a trained HuBERT's layer 6 as teacher, vs our iteration-1 spectrogram) and total training compute, per [044]. Separately on record: aves-base-bio, the 'aves' quoted in every prior headline, is tied for the WEAKEST of the six AVES checkpoints, so past comparisons used the most flattering framing for run11; the fair same-parameter bar is birdaves-biox-base at 0.8520.

**Provenance.** analysis/run15_calltype.json, analysis/aves_variants_calltype.json; weights at external/dapt/run15_combined.pt

### [052] Four times the audio per update closes most of the AVES call-type gap  ·  **CONFIRMED**

run16 -- run15's exact recipe with the four requested GPUs actually realised, so each optimiser update sees 4.0x the audio (322 s vs 80.5 s) at the same 93750 updates and lr 1e-4 -- beats run15 and run11 on 11-class call type by resolved margins (+0.0243 [+0.0099, +0.0419] and +0.0261 [+0.0095, +0.0479]) and is no longer distinguishable from any of the six AVES checkpoints, which every earlier ZF model lost to by resolved margins.

**Evidence.** Cohort 3412 clips / 11 call types / 48 birds; majority-class rate 0.1797 (Te, 613 clips). Best-layer accuracy: run16 0.8379 (L7), run15 0.8136 (L3), run11 0.8118 (L3); AVES aves-base-bio 0.8453, aves-base-all 0.8479, aves-base-core 0.8502, birdaves-biox-base 0.8520, birdaves-biox-large 0.8552, birdaves-bioxn-large 0.8453. run16 minus each AVES checkpoint (paired bird bootstrap, 2000 resamples): aves-base-bio -0.0073 [-0.0301, +0.0170], aves-base-core -0.0123 [-0.0337, +0.0130], aves-base-all -0.0100 [-0.0297, +0.0119], birdaves-biox-base -0.0141 [-0.0341, +0.0070], birdaves-biox-large -0.0173 [-0.0391, +0.0061], birdaves-bioxn-large -0.0073 [-0.0287, +0.0140] -- all intervals cross zero. For comparison run15 lost to aves-base-bio by -0.0317 [-0.0603, -0.0038] (finding 051). The gain is not a layer-picking artefact: EVERY run16 layer (L0 0.8239 ... L11 0.8203, peak L7 0.8379) beats run15's best layer (0.8136), and at run15's own layer 3 run16 scores 0.8362, still +0.0226 [+0.0082, +0.0381] over run15. 8-class: run16 0.8276 vs run15 0.8193 and run11 0.8193 (not bootstrapped). Pretraining fit: per-step training loss, same loss and same 200-class vocabulary, logs stitched across preemptions (later attempt wins where they overlap): steps 8-12k run16 2.019 vs run15 2.333, 28-32k 1.378 vs 1.754, 58-62k 1.120 vs 1.525, 88k-end 1.047 vs 1.254.

**Method.** slurm/train_run16_compute4x.sh = train_run15_combined.sh with --ntasks-per-node=4 (the fix from the 4-GPU smoke test: --gpus 4 under --ntasks=1 had realised ONE GPU in every earlier run) and --cpus-per-task=8; nothing else changed -- same corpus (combined ZF+FSD50K, 224.42 h, k=200 labels), lr, warmup 3125, max-updates 93750, feature-weight 0, virtual-chunk-seconds 20, HUBERT_NORMALIZE_INPUT=0. Realised compute verified from checkpoints, not assumed: 2507.1 steps/epoch (run15 10032) -> 224.42 h / 2507.1 = 322.2 s of audio per update, 8,392 h seen in total (run15 2,097 h; AVES 19,444 h, so run16 is at 43% of AVES). Log shows MEMBER 1/4..4/4. Weights exported with export_weights.py (bit-identical, max|diff| 0 at three clip lengths). Scored by zfeval/experiments/run15_calltype.py --tag run16_compute4x (same encode, same leave-birds-out StratifiedGroupKFold(5) seed 0, same mean-pooled multinomial logistic probe as every AVES row). Intervals from zfeval/experiments/run16_calltype_bootstrap.py, every model at its own best 11-class layer.

**Caveats.** "Not distinguishable" is NOT "tied": every AVES point estimate is still ahead, by 0.007 to 0.017, and the intervals are about 0.045 wide, so a real gap of that size could not be resolved here. The lever is "4x audio per update at fixed updates and lr", which bundles a 4x larger effective batch (less gradient noise), 4x more total audio seen, and 4.0x more passes over the same corpus (37.4 vs 9.35); this experiment cannot separate them -- but it is the same bundle AVES used (update_freq 8 on one GPU). One training seed per arm: the bootstrap resamples birds, not pretraining runs, so seed-to-seed variation of the encoder is unmeasured for every model in the table. run16's layer 7 was selected on the reported metric (finding 046); the layer-3 check above bounds that. Call type only; detection and both holdouts are finding 053 (in-distribution detection improves, BirdPark does not). Two preemptions resumed from epoch-end checkpoints; step counts per epoch are unchanged across resumes, so no batches were replayed. Does NOT say corpus size is irrelevant at higher compute -- 051 held compute fixed. The logged train_masked_accuracy is NOT usable for any cross-run comparison in this project: mask_stats accumulate from process start and are reset only in on_validation_end, which never runs, so the value is a running mean since the last (re)start -- it lags and jumps at each resume (run16's jumps at step 30085, run15's at ~10030). This also undercuts the masked-accuracy numbers quoted in 051's caveats, which that finding had already declared incomparable.

**Provenance.** Savio job 39203444 (preempted twice, auto-requeued, completed 2026-09-24 15:18, exit 0); checkpoint temp_train_run16_compute4x/.../epoch=37-step=93750.ckpt; weights external/dapt/run16_compute4x.pt; analysis/run16_compute4x_calltype.json, analysis/run16_bootstrap.json; metrics savio_artifacts/metrics/run16/; log savio_artifacts/logs/train_run16_compute4x_39203444.log

### [053] run16's compute gain reaches in-distribution detection but not the BirdPark holdout  ·  **CONFIRMED**

run16 (4x audio per update, finding 052) beats run11 on in-distribution ZF vocalisation detection by a small resolved margin (AUC 0.9718 vs 0.9687, +0.0031, 95% [+0.0017, +0.0052]) but does not beat it on the BirdPark holdout, where at the pre-committed arm it scores 0.8085 against run11's 0.8648 (-0.0563, 95% [-0.0940, +0.0122] at the honest 30 s block, not distinguishable), and the chick holdout cannot separate run16, run15, run11 or aves-base-bio.

**Evidence.** ZF in-distribution (90,061 frames, prevalence 0.1242; contiguous 60 s blocks within one recording): run16 AUC 0.9718 / AP 0.8741 (native L0) -- highest of any 94M-parameter model in the table; only birdaves-bioxn-large (315M) is higher at 0.9724. run11 0.9687 / 0.8681; run15 0.9696 / 0.8714, not distinguishable from run11 (95% [-0.0005, +0.0044]). log-energy 0.7837. BirdPark (5,925 frames, prevalence 0.4290, 4.0 independent 30 s blocks), pre-committed arm: run16 0.8085 AUC / 0.7451 AP, run15 0.7476 / 0.7091, run11 0.8648 / 0.8121, AVES family 0.8665-0.8994, log-energy 0.8542. At block=500, which the script itself flags as understating width, run16 is resolved worse (95% [-0.0908, -0.0153]); run15 is resolved worse at both block sizes (0.7476 vs 0.8648, 95% [-0.2545, -0.0275] at 1500). On the matched (chunk-normalised) arm -- the preprocessing finding 049 shows carries transfer for 23 of 23 models -- run16 is 0.8511 / 0.7855 and run15 0.8042 / 0.7112 (audit columns, not headlined). Chick holdout (10 chicks with both classes, majority-class 0.600): Be/LT AUC run16 0.9936, run15 0.9940, run11 0.9858, aves-base-bio 0.9932; run16 minus run11 +0.0078, 95% [-0.0004, +0.0224]. AMI(k=4) run16 0.5095, run11 0.5361, run15 0.4637, aves-base-bio 0.3873; interval for run16 vs run11 spans [-0.1364, +0.1343].

**Method.** zfeval/experiments/detection_variants.py --models run15_combined,run16_compute4x (both normalisations, six-layer relative-depth grid, layer pre-committed by out-of-fold ZF AUC and frozen for BirdPark, block bootstrap at 1500 and 500 frames), then chick_holdout_variants.py --models run15_combined,run16_compute4x (arm read from the detection pre-commitment, never chosen on chicks) and --bootstrap --baseline run11 (resampling unit = chick).

**Caveats.** BirdPark is four independent blocks, and the pre-commitment rule chose run16's raw-waveform arm over the normalised one on a ZF AUC difference of 0.0016 -- a coin flip that costs 0.043 of BirdPark AUC (finding 049's failure mode, again). So "does not beat run11 on BirdPark" is the defensible reading; "is worse than run11" is not established at the honest block size. Within our own models the direction is consistent: run15 -> run16 raised BirdPark AUC on both arms (native 0.7476 -> 0.8085, matched 0.8042 -> 0.8511), so compute helped here too; what lowered it relative to run11 arrived with run15's change (combined corpus + k=200), not with compute. That attribution is suggestive only -- one holdout, four blocks. The chick bootstrap's printed "dAMI" column is the MEAN of resampled differences (k-means refit at n_init=3 on chick-duplicated sets), not the observed difference; the observed values are quoted above. The same holds for detection: zfeval.metrics.paired_bootstrap returns delta = mean of resampled differences, which for the 4-block BirdPark set is -0.0438 against an observed -0.0563; every difference in this finding is the OBSERVED one, with the percentile interval from the bootstrap. Be/LT is at ceiling for every encoder and works only as a degradation detector.

**Provenance.** analysis/detection_variants.json, analysis/chick_holdout_variants.json; features features/detvar/{zf,bp}_run1{5_combined,6_compute4x}_{matched,native}.npy; weights external/dapt/run15_combined.pt, external/dapt/run16_compute4x.pt
...
### [041] REFUTED (see 044): run11 is not under-trained, it is under-diversified  ·  **REFUTED**

run11 and AVES got comparable optimiser budgets. AVES saw 2.13x more audio because its batch was twice as large, but it spread that over a corpus 3.6x bigger, so run11 actually made 1.7x MORE passes over its own data. More training on the same 100 h is therefore not the missing ingredient.

**Evidence.** run11 93,750 updates x 350 audio-seconds/update = 9,114.6 h of audio seen, over a ~100 h corpus = 91.1 epochs. aves-base-bio 100,000 updates x 700 audio-seconds/update = 19,444.4 h, over a 360 h corpus = 54.0 epochs. Ratios AVES/run11: steps 1.07x, audio 2.13x, corpus 3.60x, epochs 0.59x. Peak LR 1e-4 vs 2e-4. Warmup fraction 0.033 vs 0.320 (fairseq stock 32k warmup left in place after cutting the run to 100k). k-means clusters 100 vs 200. AVES wall clock 24.6 h on ONE GPU; run11 requested 12 h on 4 GPUs so <= 48 GPU-hours, actual not recorded locally.

**Method.** Every figure machine-read, none typed. run11 from the submitted slurm script (slurm/train_iter7_long_lowprio.sh, whose header names it run11) with train.py argparse defaults for unset flags; the 87.5 s/batch default times 4 DDP ranks gives 350 s/update, because BucketizeBatchSampler builds one 87.5 s batch per rank and DistributedBatchSampler shards across ranks (lightning_modules.py:378-386). AVES from the fairseq cfg inside the 2022 checkpoint, read by stubbing the fairseq/omegaconf modules: max_tokens 1.4e6 / 16000 x update_freq 8 x world_size 1 = 700 s. Self-check that makes the whole table trustworthy: epochs DERIVED as (steps x audio/step) / corpus-hours gives 54.0 for AVES, against the epoch counter fairseq itself wrote into the checkpoint, 55 -- 1.8% apart. That agreement independently corroborates both the 700 s/update arithmetic and the 360 h corpus figure taken from the paper, and the script asserts it.

**Caveats.** The two corpus-hour figures are the only quantities not machine-read -- ~100 h is the release README's and 360 h is Hagiwara 2023 Table 1 -- but the epoch self-check above constrains the AVES one. run11's true wall clock and whether it was preempted and resumed are on Savio, not here, so the 48 GPU-hour figure is a ceiling and not a measurement. Audio-seconds seen counts repeats, so it is a compute measure and not an information measure; the epochs row is the one that speaks to diversity. Architecture is not in this table because it is not a difference: both models are 94,370,944 parameters with bit-identical state_dict keys.
